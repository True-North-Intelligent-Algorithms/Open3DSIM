function [dataparams,psf] = find_illumination_pattern2(Snoisy,dataparams)
...
NotchFilter0=getotfAtt(numpixelsx,cyclesPerMicron,0.2*cutoff,0,0);
NotchFilter1=NotchFilter0.*Mask1;
Mask2=double(rad<=0.8*(cutoff/cyclesPerMicron+1));
...
for I = 1:dataparams.numangles
    ...
    [peak,~] = fitPeak(separateII(:,:,1)/(max(max(abs(separateII(:,:,1))))),separateII(:,:,2)/(max(max(abs(separateII(:,:,2))))),1,bn1,OtfProvider,-kx,-ky,overlap,step,cntrl);
